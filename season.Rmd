...
edges <-
  bind_rows(
    mutate(rename(select(point, order, team, assist2, assist1), from = assist2, to = assist1), type = "A2"),
    mutate(rename(select(point, order, team, assist1, scorer), from = assist1, to = scorer), type = "A1")
  ) %>%
  mutate(
